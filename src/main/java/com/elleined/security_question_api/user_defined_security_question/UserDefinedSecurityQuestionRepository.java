...
public interface UserDefinedSecurityQuestionRepository extends CrudRepository<UserDefinedSecurityQuestion, UUID> {
    ...
    @Query("CALL user_defined_security_question_find_all(:resourceId, :page, :size)")
    List<UserDefinedSecurityQuestionDTO> findAll(@Param("resourceId") UUID resourceId,
                                                 @Param("page") int page,
                                                 @Param("size") int size);

    @Query("SELECT user_defined_security_question_find_all_total(:resourceId)")
    int findAllTotal(@Param("resourceId") UUID resourceId);

    @Query("SELECT user_defined_security_question_find_answer(:id, :resourceId)")
    String findAnswer(@Param("id") UUID id,
                      @Param("resourceId") UUID resourceId);

    @Modifying
    @Transactional
    @Query("CALL user_defined_security_question_save(:resourceId, :question, :answer)")
    ...
    @Modifying
    @Transactional
    @Query("CALL user_defined_security_question_update(:id, :resourceId, :answer)")
